...
def get_integer_input():
    ...
    while True:
        ...
        if user_input.strip() and user_input.isdigit(): # Check that the input exists and its a number
            user_input = int(user_input) # Convert the input to an integer
            if user_input <= 3: # Make sure that the input is not bigger than 3
                return user_input
            else:
...
def generate_title():
    ...
    # Get the number of words from the user and return a message if the input is a number between 1 and 3
